...
async def create_book(book_data: BookCreate, database: Session = Depends(get_db)) -> Book:
    ...
    db_book = Book(
        price=book_data.price,        release=book_data.release,        time=book_data.time,        pages=book_data.pages,        stock=book_data.stock,        title=book_data.title        )
...
async def bulk_create_book(items: list[BookCreate], database: Session = Depends(get_db)) -> dict:
    ...
    for idx, item_data in enumerate(items):
        try:
            ...
            db_book = Book(
                price=item_data.price,                release=item_data.release,                time=item_data.time,                pages=item_data.pages,                stock=item_data.stock,                title=item_data.title            )
            database.add(db_book)
            database.flush()  # Get ID without committing
